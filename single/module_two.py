...
import ultraimport
cno_module_2 = ultraimport('__dir__/../campaign_names.py', 'single_cno_module_2')
...
def get_table(cols: list, keyword_type: str, keywords: list, neg_keywords: list = [], skus: list = []):
    items = []
    items.append({
...
def is_from_row(row, value, kws):
    ...
    rows_values = ['SKU', 'ASIN', 'Match Type', 'SKU Group Name']
...
def make_campaign_name(row, campaign_name_order, kws):
    campaign_name = ''
    i = 0
    while True:
        try:
            if i == 0:
                campaign_name += f"{is_from_row(row, campaign_name_order[i], kws)}"
            else:
                campaign_name += f"_{is_from_row(row, campaign_name_order[i], kws)}"
        except:
            break
        i += 1
    return campaign_name

def modify_table(row, x_table, kw_type, kws, neg_info: list = [], campaign_name_order: dict = cno_module_2, pa_df = None):
    #Generating the campaign name
    campaign_name = make_campaign_name(row, campaign_name_order, kws)
    ...
    #sku and asin
    if type(pa_df) != types.NoneType:
        console_sc_vc = lambda x, y, value: y if x == value else None
        x_table.loc[x_table['Entity'] == 'Product Ad', 'SKU'] =  console_sc_vc(row['Console'], pa_df[0].to_list(), 'SC')
        x_table.loc[x_table['Entity'] == 'Product Ad', 'ASIN'] = console_sc_vc(row['Console'], pa_df[1].to_list(), 'VC')
    else:
