...
import ultraimport
cno_module_7 = ultraimport('__dir__/../campaign_names.py', 'single_cno_module_7_8')
...
def saving_module_errors(error: dict, row, error_messages: list, warning_messages: list):
    if 'bid_mod' in error['loc']:
        ...
        error_messages.append(f"Row {row.name} Audience Targeting Error: Must be exactly one of views-Advertised products, views-Related to advertised products, purchases-Advertised products, purchases-Related to advertised products")
...
class Module7(hep.CommonObj):
    ...
    def validate_audience_targeting(cls, input_data: str | float):
        audience_targetings = ['views-Advertised products',
        'views-Related to advertised products', 'purchases-Advertised products', 'purchases-Related to advertised products']
