...
import ultraimport
cno_module_5 = ultraimport('__dir__/../campaign_names.py', 'single_cno_module_5')
...
@hep.add_ps_feature
def get_table(out_cols, targets: list = [], skus: list = []):
    items = []
    items.append({
...
def is_from_row(row, value):
    ...
    if value in row_values:
        ...
        return f"{cost_type_campagin(row['Bid Optimization'])}_ASIN"
    elif value == 'Defense':
        if row['Defense'] == 'yes':
            return 'Defense'
        else:
